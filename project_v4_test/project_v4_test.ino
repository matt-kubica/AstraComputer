...
static void debounceUpButton();
static void debounceDownButton();
...
void setup() {
  ...
  //T4 registers configuration
  //responsible for checking for buttons
  //-----------------------------------------------------------------------------------------------
  TCCR4A = 0;
  
  TCCR4B |= (1 << CS40);    // prescaler = 1024
  TCCR4B |= (1 << CS41);
  TCCR4B &= ~(1 << CS42); 
  TCCR4B |= (1 << CS43);
...
//----------------------------------------------------------------------------------------------------------------------------------------------------
void loop() { 
  if(reload or upButtonPressed or downButtonPressed) {
...
static void debounceUpButton() {
  ...
  uint8_t currentState = digitalRead(UP_BUTTON_PIN);
...
static void debounceDownButton() {
  static uint8_t count = 0;
  static uint8_t buttonState = 0;
  uint8_t currentState = digitalRead(DOWN_BUTTON_PIN);
